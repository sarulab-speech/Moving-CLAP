...
class Encoder(nn.Module):
    ...
    def forward(self, x, return_temporal=True):
        ...
        # Prepare for CNN: permute to (batch, channels, time, freq)
        ...
        x = x.permute(0, 2, 1, 3).contiguous()  # (batch, time_steps, cnn_channels, freq_bins_reduced)
        x = x.view(batch_size, time_steps, -1)  # (batch, time_steps, cnn_channels * freq_bins_reduced)
        # Project to middle_features dimension
        x = self.middle_linear(x)  # (batch, time_steps, middle_features)
        # GRU processing
        x, _ = self.gru(x)  # (batch, time_steps, output_features)
        if return_temporal:
            return x  # (batch, time_steps, output_features)
        else:
            x = torch.mean(x, dim=1)  # (batch, output_features)
            return x